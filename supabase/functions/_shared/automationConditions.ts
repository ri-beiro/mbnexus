// Deno-runtime mirror of src/features/automation/conditionLogic.ts.
// Edge Functions run in a separate Deno runtime from the Vite/Node frontend
// build, so this can't be imported directly — keep the two in sync by hand
// if the rule set changes; src/features/automation/conditionLogic.test.ts is
